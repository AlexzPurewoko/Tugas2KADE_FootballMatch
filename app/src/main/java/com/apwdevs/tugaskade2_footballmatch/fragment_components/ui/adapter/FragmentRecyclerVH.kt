...
class FragmentRecyclerVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bindItem(item: MatchTeamLeagueData, listener: (MatchTeamLeagueData) -> Unit) {
        home_team_name.text = item.strHomeTeam
        home_team_score.text = if (item.intHomeScore == null) "0" else item.intHomeScore
        away_team_name.text = item.strAwayTeam
        away_team_score.text = if (item.intAwayScore == null) "0" else item.intAwayScore
